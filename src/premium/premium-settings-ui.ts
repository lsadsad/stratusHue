...
export class PremiumSettingsUI {
  ...
  async render(): Promise<void> {
    ...
    const featureStatus = premiumFeatures.getFeatureStatus();
  ...
  private async renderFeatureCategories(featureStatus: any): Promise<string> {
    const categories = Object.entries(FEATURE_CATEGORIES);
    
    const categoryHtml = await Promise.all(
      categories.map(async ([categoryKey, category]) => {
        const features = getFeaturesByCategory(categoryKey as keyof typeof FEATURE_CATEGORIES);
        
        if (features.length === 0) return '';

        const featureItems = features.map(({ feature, name, description, icon }) => {
          const isAvailable = featureStatus.features[feature];
          const isInFreeTier = feature === PREMIUM_FEATURES.UNLIMITED_BOOKMARKS || 
                              feature === PREMIUM_FEATURES.CUSTOM_EMOJI_SETS;
  ...
  private renderLimitInfo(feature: string, featureStatus: any): string {
    const limits = featureStatus.limits;
    
    if (feature === PREMIUM_FEATURES.UNLIMITED_BOOKMARKS) {
    ...
    if (feature === PREMIUM_FEATURES.CUSTOM_EMOJI_SETS) {
  ...
  private renderUpgradeSection(featureStatus: any): string {
  ...
  private attachEventListeners(): void {
    ...
    document.getElementById('deactivate-license')?.addEventListener('click', () => {
    ...
    try {
      ...
      const usageData = await Promise.all(
        features.map(async (feature) => {
          const usage = await premiumFeatures.getFeatureUsage(feature);
          const description = premiumFeatures.getFeatureDescription(feature);
      ...
      const statsHtml = usageData.map(({ feature, usage, description }) => `
        <div class="usage-item">
          <div class="usage-icon">${description?.icon || '📊'}</div>
          <div class="usage-info">
            <div class="usage-name">${description?.name || feature}</div>
  ...
  private async handleDeactivateLicense(): Promise<void> {
    if (confirm('Are you sure you want to deactivate your premium license?')) {
      await premiumFeatures.deactivateLicense();
...
export const PREMIUM_SETTINGS_CSS = `
  .premium-settings {
