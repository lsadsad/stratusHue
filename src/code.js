...
async function saveCurrentAnchor(bookmarkId) {
    try {
...
async function addToRecentHistory(bookmarkId) {
    try {
...
async function validateRecentHistory() {
    try {
...
async function detectCurrentAnchorFromSelection() {
...
async function validateCurrentAnchor() {
...
async function updateBookmarksForPage(pageId, newPageName) {
    const bookmarks = await getBookmarks();
...
async function getBookmarks() {
    ...
    const migratedBookmarks = await Promise.all(bookmarks.map(async (bookmark) => {
        const b = bookmark;
        if (!b.pageName) {
            try {
...
async function updateAndSaveBookmarks(bookmarks) {
...
async function updateBookmarkIfExists(layerId, newName) {
    const bookmarks = await getBookmarks();
...
async function autoValidateBookmarks() {
    const now = Date.now();
...
async function cleanupBookmark(bookmarkId) {
    const bookmarks = await getBookmarks();
...
async function navigateToNode(node, recordHistory = true) {
    const targetPage = getContainingPage(node);
    if (targetPage) {
    ...
    try {
        if (entry.nodeId) {
            ...
            if (node && 'name' in node) {
                await navigateToNode(node, false);
            }
            else {
...
// --- Plugin UI Setup ---
// Track current dialog width and height so we can toggle width while preserving height
let currentUiWidth = 240;
...
async function validateAndSyncBookmarks() {
    const bookmarks = await getBookmarks();
...
async function handleSaveBookmark(selectedLayers) {
...
async function handleJumpToBookmark(bookmarkId) {
...
async function updateLayerEmojis(layers, emoji) {
    if (layers.length === 0)
        return false;
    const bookmarks = await getBookmarks();
...
async function updatePageEmoji(page, emoji) {
    var _a;
...
async function handleAddEmoji(selectedLayers, emoji) {
    if (selectedLayers.length > 0) {
...
async function clearLayerEmojis(layers) {
...
async function clearPageEmoji(page) {
    const originalName = page.name;
    const parts = parsePageTitleParts(originalName);
...
async function handleClearEmoji(selectedLayers) {
    if (selectedLayers.length > 0) {
 ...
 */
async function handleCollapseLayers(selectedLayers) {
    if (selectedLayers.length === 0) {
...
async function handleNavigateEmojiSet(direction) {
    const selectedLayers = figma.currentPage.selection;
    const hasLayerSelected = selectedLayers.length > 0;
    if (hasLayerSelected) {
...
figma.ui.onmessage = async (msg) => {
    ...
    try {
        switch (msg.type) {
            ...
            case 'toggle-width': {
                // Toggle between 240 and 184 while preserving height
